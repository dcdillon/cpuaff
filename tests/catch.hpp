...
struct ResultDisposition
{
    enum Flags
    {
        ...
        ContinueOnFailure =
            0x01,            // Failures fail test, but execution continues
...
{
    return Evaluator< T *, T *, Op >::evaluate(lhs,
...
class ExpressionLhs
{
   ...
   public:
    ...
    // If more complex compositions are required then place the sub expression
    // in parentheses
    template < typename RhsT >
    STATIC_ASSERT_Expression_Too_Complex_Please_Rewrite_As_Binary_Comparison &
        operator+(RhsT const &);
...
struct TextAttributes
{
    ...
    std::size_t width;  // maximum width of text, including indent. Longer text
                        // will wrap
    ...
    void argSynopsis(std::ostream &os) const
    {
        for (int i = 1; i <= m_highestSpecifiedArgPosition; ++i)
        {
            ...
            if (it != m_positionalArgs.end())
                ...
                throw std::logic_error(
                    "non consecutive positional arguments with no floating "
                    "args");
        }
   ...
   private:  // IResultCapture
                              ...
                              double _durationInSeconds)
    {
        ...
        m_reporter->sectionEnded(SectionStats(
            info, assertions, _durationInSeconds, missingAssertions));
        m_messages.clear();
    }
    ...
    , properties(None)
{
         ...
         ++it)
    {
        ...
        properties = static_cast< SpecialProperties >(
...
class ReporterRegistrar
{
    class ReporterFactory : public IReporterFactory
    {
        ...
        // to register
        // your custom reporter class be aware that the native reporter
        // interface has changed
        // to IStreamingReporter. The "legacy" interface, IReporter, is still
        // supported via
        // an adapter. Just use REGISTER_LEGACY_REPORTER to take advantage of
        // the adapter.
        // However please consider updating to the new interface as the old one
        // is now
        // deprecated and will probably be removed quite soon!
        // Please contact me via github if you have any questions at all about
        // this.
        // In fact, ideally, please contact me anyway to let me know you've hit
        // this - as I have
        // no idea who is actually using custom reporters at all (possibly
        // no-one!).
        // The new interface is designed to minimise exposure to interface
            ...
            , printInfoMessages(_printInfoMessages)
        {
            switch (result.getResultType())
            {
                ...
                case ResultWas::DidntThrowException:
                    ...
                    passOrFail = "FAILED";
                    messageLabel =
                        "because no exception was thrown where one was "
                        "expected";
                    break;
                case ResultWas::Info:
                    messageLabel = "info";
